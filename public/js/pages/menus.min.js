...
var Menus = function () {
  ...
  var initFilter = function initFilter() {
    $('#modalAdd').on('show.bs.modal', function (e) {
      $("h5#modalAddLabel").text("Tambah Menu");
      $.ajax({
        ...
        success: function success(result) {
          ...
          $.ajax({
            ...
            success: function success(result) {
              ...
              $("#parentMenuEdit").val(result.response.parentId).trigger("change");
              $.each(result.response.menu_action, function (index, value) {
                $('#formEdit input[name="actions[]"][value="' + value.actionId + '"]').prop('checked', true);
              });
            },
            error: function error(eventError, textStatus, errorThrown) {
              Swal.fire({
